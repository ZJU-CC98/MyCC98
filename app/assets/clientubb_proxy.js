...
var emotnum=72;
var isSimple=false;
var emotdir='http://hz.cc98.lifetoy.org/emot/';
var icondir='http://hz.cc98.lifetoy.org/images/files/';
var tdclass;
...
var ubb = {
	...
	img:function(str,type){
		var pattern;
		pattern = /\[img\]http:\/\/(hz\.cc98\.lifetoy\.org|cc\.zju\.edu\.cn)\/(.[^\[]*)\[\/img\]/gi;
		str = str.replace(pattern, '<span style="cursor:pointer;text-decoration:line-through">该文件已在地震中消失了</span>');
		if(type == 0){
			if(canimgsign){
				str = str.replace(/\[img\](http:\/\/([a-z]+).cc98.org\/(.[^\[]*))\[\/img\]/ig, '<img src="$1" style="border:0;" onload="if (this.width > 600 || this.height > 200) this.src = \'pic/toobig.jpg\'" />');
			...
			if (canimgcode){
				//share image
				pattern = /\[IMG=0\](http:\/\/share\.cc98\.lifetoy\.org\/[A-Za-z0-9]+)(.file)?\[\/IMG\]/gi;
				str=str.replace(pattern,"<br /><a onfocus=this.blur() href=\"$1\" target=_blank><img src=\"$1\" border=0 alt=\"按此在新窗口浏览图片\" class=\"resizeable\" /></a>");
				pattern = /\[IMG(=1)?\](http:\/\/share\.cc98\.lifetoy\.org\/[A-Za-z0-9]+)(.file)?\[\/IMG\]/gi;
				...
				pattern=/\[UPLOAD=(gif|jpg|jpeg|bmp|png)\](http:\/\/hz\.file\.cc98\.lifetoy\.org\/.[^\[\'\"\:\(\)]*)(gif|jpg|jpeg|bmp|png)\[\/UPLOAD\]/gi;
				str= str.replace(pattern,"<br /><a href=\"$2$1\" target=\"_blank\"><img src=\"$2$1\" border=0 alt=\"按此在新窗口浏览图片\" class=\"resizeable\"></a>");
				pattern=/\[UPLOAD=(gif|jpg|jpeg|bmp|png),0\](http:\/\/hz\.file\.cc98\.lifetoy\.org\/.[^\[\'\"\:\(\)]*)(gif|jpg|jpeg|bmp|png)\[\/UPLOAD\]/gi;
				str= str.replace(pattern,"<br /><a href=\"$2$1\" target=\"_blank\"><img src=\"$2$1\" border=0 alt=\"按此在新窗口浏览图片\" class=\"resizeable\"></a>");
				pattern=/\[UPLOAD=(gif|jpg|jpeg|bmp|png),1\](http:\/\/hz\.file\.cc98\.lifetoy\.org\/.[^\[\'\"\:\(\)]*)(gif|jpg|jpeg|bmp|png)\[\/UPLOAD\]/gi;
				...
				pattern=/\[UPLOAD=(gif|jpg|jpeg|bmp|png)([,]*)([01]*)\](http:\/\/hz\.file\.cc98\.lifetoy\.org\/.[^\[\'\"\:\(\)]*)(gif|jpg|jpeg|bmp|png)\[\/UPLOAD\]/gi;
	...
	file:function(str){
		//pattern=/\[UPLOAD=txt([,]*)([01]*)\]http:\/\/hz\.file\.cc98\.lifetoy\.org\/(uploadfile\/.[^\[\'\"\:\(\)]*)\[\/UPLOAD\]/gi;
		//str = str.replace(pattern,'<img src="'+icondir+'txt.gif" border="0" /><a href="filedown.asp?url=$3" target="_self">点击下载文件</a>');
		pattern=/\[UPLOAD=(.[^\[\'\"\:\(\)]*?)([,]*)([01]*)\](http:\/\/hz\.file\.cc98\.lifetoy\.org\/.[^\[\'\"\:\(\)]*)\[\/UPLOAD\]/gi;
	...
	table:function(str){
		return str.replace(/\[table=(.*?)\](.*?)\[\/table\]/gi,"<table style=\"width:100%\" cellpadding=5 cellspacing=1 class=tableborder1><tr><td bgcolor=\"#999999\"><font color=\"#FFFFFF\"><b>$1</b></font></td></tr><TR><TD class="+tdclass+" width=\"100%\"><div style=\"overflow:auto;height:240;\">$2</div></td></tr></table>");
	},
	url:function(str, signatureMode){
		...
		if (!signatureMode) {
			...
			//pattern=/\[url\](http:\/\/hz\.file\.cc98\.lifetoy\.org\/(uploadfile\/.[^\[\'\"\:\(\)]*)\.txt)\[\/url\]/gi;
			//str = str.replace(pattern,'<a href="filedown.asp?url=$2.txt" target="_blank">$1</a>');
			//pattern=/\[url\](http:\/\/hz\.cc98\.lifetoy\.org\/(uploadfile\/.[^\[\'\"\:\(\)]*)\.txt)\[\/url\]/gi;
			//str = str.replace(pattern,'<a href="filedown.asp?url=$2.txt" target="_blank">$1</a>');
			//pattern=/\[url\](http:\/\/10\.10\.98\.98\/(uploadfile\/.[^\[\'\"\:\(\)]*)\.txt)\[\/url\]/gi;
			//str = str.replace(pattern,'<a href="filedown.asp?url=$2.txt" target="_blank">$1</a>');
	
			str= str.replace(/\[url\]http\:\/\/(hz\.cc98\.lifetoy\.org|10\.10\.98\.98|cc\.zju\.edu\.cn)\/(.[^\[\'\"\:\(\)]*)\[\/url\]/gi, "<a href=\"$2\" target=\"_blank\">$2</a>");
			str= str.replace(/\[url=http\:\/\/(hz\.cc98\.lifetoy\.org|10\.10\.98\.98|cc\.zju\.edu\.cn)\/(.[^\[\'\"\:\(\)]*)\](.*?)\[\/url\]/gi, "<a href=\"$2\" target=\"_blank\">$3</a>");
			...
			//自动识别网址
			//pattern = /http:\/\/(hz\.cc98\.lifetoy\.org|10\.10\.98\.98|cc\.zju\.edu\.cn)\/([A-Za-z0-9\.\/=\?%\-&_~`@\':+!;#]+)/gi;
			...
			str = str.replace(/\[url\](http\:\/\/((([a-z]+)\.cc98\.lifetoy\.org)|(([a-z\.]+)\.zju\.edu\.cn))\/(.[^\[\'\"\:\(\)]*))\[\/url\]/ig, '<a href="$1" target="_blank">$1</a>');
			str = str.replace(/\[url=(http\:\/\/((([a-z]+)\.cc98\.lifetoy\.org)|(([a-z\.]+)\.zju\.edu\.cn))\/(.[^\[\'\"\:\(\)]*))\](.*?)\[\/url\]/ig, '<a href="$1" target="_blank">$8</a>');
			...
			//pattern = /http:\/\/(hz\.cc98\.lifetoy\.org|10\.10\.98\.98|cc\.zju\.edu\.cn)\/([A-Za-z0-9\.\/=\?%\-&_~`@\':+!;#]+)/gi;
			//str = str.replace(pattern, '<a target="_blank" href="$2">$2</a> &nbsp;');
			pattern = /^((http|https|ftp|rtsp|mms):(\/\/|\\\\)((([a-z]+)\.cc98\.lifetoy\.org)|(([a-z\.]+)\.zju\.edu\.cn))\/[A-Za-z0-9\.\/=\?%\-&_~`@\':+!;#]+)/ig;
			str = str.replace(pattern, '<img align="absmiddle" src="pic/url.gif" style="border:0;" /><a href="$1" target="_blank">$1</a>');
			pattern = /((http|https|ftp|rtsp|mms):(\/\/|\\\\)((([a-z]+)\.cc98\.lifetoy\.org)|(([a-z\.]+)\.zju\.edu\.cn))\/[A-Za-z0-9\.\/=\?%\-&_~`@\':+!;#]+)<BR>/ig;
			str = str.replace(pattern, '<img align="absmiddle" src="pic/url.gif" style="border:0;" /><a href="$1" target="_blank">$1</a><br />');
			pattern = /([^>=\"]|<BR>)((http|https|ftp|rtsp|mms):(\/\/|\\\\)((([a-z]+)\.cc98\.lifetoy\.org)|(([a-z\.]+)\.zju\.edu\.cn))\/[A-Za-z0-9\.\/=\?%\-&_~`@\':+!;#]+)/ig;
