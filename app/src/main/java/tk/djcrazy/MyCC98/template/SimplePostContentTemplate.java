...
public class SimplePostContentTemplate implements PostcontentTemplate {
	
	private static PostcontentTemplate template = new SimplePostContentTemplate();
	...
	public String genContent(Context context, List<PostContentEntity> list ,int page) {
		...
		builder.append(last);
		Log.d("SimplePostContentTemplate", builder.toString());
	...
	private void prepareContent(Context context) {
		if (mContent==null) {
			try {
				mContent = StreamUtils.Stream2String(context.getAssets().open("template/simple_post_content_template.html"));
