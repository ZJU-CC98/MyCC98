...
public class LifetoyPostContentTemplate implements PostcontentTemplate {
	
	private static PostcontentTemplate template = new LifetoyPostContentTemplate();
	...
	private void prepareContent(Context context) {
		if (mContent==null) {
			try {
				mContent = StreamUtils.Stream2String(context.getAssets().open("template/lifetoy_post_content_template.html"));
